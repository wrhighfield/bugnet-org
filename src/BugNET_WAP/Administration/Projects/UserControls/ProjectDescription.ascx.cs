...
namespace BugNET.Administration.Projects.UserControls
{
    ...
    /// </summary>
    public partial class ProjectDescription : BugNetUserControl, IEditProjectControl
    {
        ...
        public void Initialize()
        {
            ...
            ProjectManager.Items.Insert(0, new ListItem(GetLocalString("SelectUser"), ""));
            ...
            if (ProjectId > Globals.NewId)
            {
                ...
                if (projectToUpdate != null)
                {
                    ...
                    ProjectImage.ImageUrl = $"~/DownloadAttachment.axd?id={ProjectId}&mode=project";
                }
            }
            else
            {
        ...
        public bool Update()
        {
            if (Page.IsValid)
            {
                ...
                if (uploadFile.ContentLength > 0)
                {
                    ...
                    foreach (var newfileType in allowedFileTypes
                                 .Select(fileType => fileType.Substring(fileType.LastIndexOf(".")))
                                 .Where(newfileType => newfileType.CompareTo(fileExt) == 0)) isFileOk = true;

                    //file type is not valid
                    if (!isFileOk)
                    {
                        if (Log.IsErrorEnabled)
                            Log.Error(string.Format(LoggingManager.GetErrorMessageResource("InvalidFileType"),
                                uploadedFileName));
                        return false;
                    }
                    ...
                    {
                        if (Log.IsErrorEnabled)
                            Log.Error(string.Format(LoggingManager.GetErrorMessageResource("InvalidFileName"),
                                uploadedFileName));
                        return false;
                    }
                    ...
                    projectImage = new ProjectImage(ProjectId, fileBytes, uploadedFileName, fileSize,
                        uploadFile.ContentType);
                }

                var attachmentStorageType =
                    (IssueAttachmentStorageTypes) Enum.Parse(typeof(IssueAttachmentStorageTypes),
                        AttachmentStorageType.SelectedItem.Value);
