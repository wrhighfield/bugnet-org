...
namespace BugNET.Administration.Projects.UserControls
{
    ...
    /// </summary>
    public partial class ProjectMembers : BugNetUserControl, IEditProjectControl
    {
        /// <summary>
        ...
        public void Initialize()
        {
            ...
            var projectUsers = UserManager.GetUsersByProjectId(ProjectId);
            foreach (var currentUser in projectUsers)
            {
                var matchItem = lstAllUsers.Items.FindByValue(currentUser.UserName);
                if (matchItem != null)
                {
        ...
        private void RebindProjectMembers()
        {
            ...
            ddlProjectMembers.Items.Insert(0, new ListItem(GetLocalString("SelectUser"), "0"));
        ...
        private void ddlProjectMembers_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (ddlProjectMembers.SelectedIndex > 0)
            {
                ...
                // Copy selected users into Selected Users List Box
                var UserRoles = RoleManager.GetForUser(ddlProjectMembers.SelectedValue, ProjectId);
                foreach (var CurrentRole in UserRoles)
                {
                    var matchItem = lstAllRoles.Items.FindByValue(CurrentRole.Id.ToString());
                    if (matchItem != null)
                    {
        ...
        private void Button4_Click(object sender, EventArgs e)
        {
            ...
            {
                RoleManager.RemoveUser(ddlProjectMembers.SelectedValue, int.Parse(lstSelectedRoles.SelectedValue));
        ...
        {
            Response.Redirect("~/Administration/Projects/EditProject.aspx?id=" + ProjectId);
        }
    }
}