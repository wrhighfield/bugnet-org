...
namespace BugNET.HttpModules
{
    ...
    public class AuthenticationModule : IHttpModule
    {
        ...
        private static void ContextAuthenticateRequest(object sender, EventArgs e)
        {
            ...
            //get host settings
            var enabled = HostSettingManager.Get(HostSettingNames.UserAccountSource) == "ActiveDirectory" ||
                          HostSettingManager.Get(HostSettingNames.UserAccountSource) == "WindowsSAM";
            ...
            if (user == null)
            {
                try
                {
                    ...
                    membershipUser = Membership.RequiresQuestionAndAnswer
                        ? Membership.CreateUser(HttpContext.Current.User.Identity.Name,
                            Membership.GeneratePassword(7, 2), userProperties.Email, "WindowsAuth", "WindowsAuth", true,
                            out createStatus)
                        : Membership.CreateUser(HttpContext.Current.User.Identity.Name,
                            Membership.GeneratePassword(7, 2), userProperties.Email);
                ...
                catch (Exception ex)
                {
                    if (Log.IsErrorEnabled)
                        Log.Error(
                            $"Unable to add new user '{HttpContext.Current.User.Identity.Name}' to BugNET application. Authentication Type='{HttpContext.Current.User.Identity.AuthenticationType}'.",
                            ex);
                }
            }
            else
        ...
        private static UserProperties GetUserProperties(string identification)
        {
            ...
            switch (HostSettingManager.UserAccountSource)
            {
                ...
                {
                    // Setup the filter
                    var username = identification.Substring(
                        identification.LastIndexOf(@"\", StringComparison.Ordinal) + 1,
                        identification.Length - identification.LastIndexOf(@"\", StringComparison.Ordinal) - 1);
                    ...
                    if (!string.IsNullOrWhiteSpace(user.DisplayName)) userProperties.DisplayName = user.DisplayName;

                    var regexEmail = new Regex(VALID_EMAIL_REGULAR_EXPRESSION,
