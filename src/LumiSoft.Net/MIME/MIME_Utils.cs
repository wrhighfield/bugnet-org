    ...
    public class MIME_Utils
    {
        ...
        public static string DateTimeToRfc2822(DateTime dateTime)
        {
            return dateTime.ToString("ddd, dd MMM yyyy HH':'mm':'ss ",
                System.Globalization.DateTimeFormatInfo.InvariantInfo) + dateTime.ToString("zzz").Replace(":", "");
        }
        ...
        public static DateTime ParseRfc2822DateTime(string value)
        {
            ...
            try
            {
                ...
                var year = Convert.ToInt32(r.Atom());
                var hour = Convert.ToInt32(r.Atom());
                r.Char(true);
                var minute = Convert.ToInt32(r.Atom());
                var second = 0;
                ...
                else
                {
                    ...
                    else if (v == "Z") timeZoneMinutes = 0000;
                ...
                return new DateTime(timeUTC.Year, timeUTC.Month, timeUTC.Day, timeUTC.Hour, timeUTC.Minute,
                    timeUTC.Second, DateTimeKind.Utc).ToLocalTime();
            }
            catch (Exception x)
            {
                var dymmy = x.Message;
            ...
            */
            ...
            while (b > -1)
            {
                ...
                if (b == '=')
                {
                    ...
                    var nCount = msSourceStream.Read(buffer, 0, 2);
                    if (nCount == 2)
                    {
                        ...
                        else
                        {
                            try
                            {
                                msRetVal.Write(NetUtils.FromHex(buffer), 0, 1);
                            }
                            catch
                            {
