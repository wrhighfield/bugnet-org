...
namespace LumiSoft.Net.MIME
{
    ...
    public class MIME_h_Collection : IEnumerable
    {
        ...
        {
            if (index < 0 || index > m_pFields.Count) throw new ArgumentOutOfRangeException(nameof(index));
        ...
        public string ToString(MIME_Encoding_EncodedWord wordEncoder, Encoding parmetersCharset, bool reEncode)
        {
            ...
            foreach (var field in m_pFields) retVal.Append(field.ToString(wordEncoder, parmetersCharset, reEncode));
        ...
        public void Parse(SmartStream stream, Encoding encoding)
        {
            ...
            while (true)
            {
                ...
                {
                    if (currentHeader.Length > 0) Add(currentHeader.ToString());
                    m_IsModified = false;
