...
namespace LumiSoft.Net.MIME
{
    ...
    public abstract class MIME_b_SinglepartBase : MIME_b
    {
        ...
        {
            if (m_pEncodedDataStream != null) m_pEncodedDataStream.Close();
        }
        ...
        internal override void SetParent(MIME_Entity entity, bool setContentType)
        {
            ...
            if (setContentType && (Entity.ContentType == null || !string.Equals(Entity.ContentType.TypeWithSubtype,
                    MediaType, StringComparison.InvariantCultureIgnoreCase)))
        ...
        {
            if (contentTransferEncoding == null) throw new ArgumentNullException(nameof(contentTransferEncoding));
            ...
            // Owner entity has no content-type or has different content-type, just add/overwrite it.
            if (Entity.ContentType == null || !string.Equals(Entity.ContentType.TypeWithSubtype, MediaType,
        ...
        public void SetData(Stream stream, string transferEncoding)
        {
            ...
            if (string.Equals(transferEncoding, MIME_TransferEncodings.QuotedPrintable,
                    StringComparison.InvariantCultureIgnoreCase))
            ...
            else if (string.Equals(transferEncoding, MIME_TransferEncodings.Base64,
                         StringComparison.InvariantCultureIgnoreCase))
                using (var fs = new MemoryStreamEx(32000))
                {
            ...
            else if (string.Equals(transferEncoding, MIME_TransferEncodings.Binary,
                         StringComparison.InvariantCultureIgnoreCase))
                SetEncodedData(transferEncoding, stream);
            else if (string.Equals(transferEncoding, MIME_TransferEncodings.EightBit,
                         StringComparison.InvariantCultureIgnoreCase))
                SetEncodedData(transferEncoding, stream);
            else if (string.Equals(transferEncoding, MIME_TransferEncodings.SevenBit,
                         StringComparison.InvariantCultureIgnoreCase))
                SetEncodedData(transferEncoding, stream);
            else
