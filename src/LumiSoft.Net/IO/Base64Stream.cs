...
namespace LumiSoft.Net.IO
{
    ...
    public class Base64Stream : Stream, IDisposable
    {
        ...
        private static readonly byte[] BASE64_ENCODE_TABLE = new byte[]
        {
            ...
            (byte) 'T',
            (byte) 'U', (byte) 'V', (byte) 'W', (byte) 'X', (byte) 'Y', (byte) 'Z', (byte) 'a', (byte) 'b', (byte) 'c',
            (byte) 'd',
        ...
        public Base64Stream(Stream stream, bool owner, bool addLineBreaks) : this(stream, owner, addLineBreaks,
        ...
        {
            if (m_IsDisposed) throw new ObjectDisposedException("Base64Stream");
        }
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            if (m_DecodedBlockCount - m_DecodedBlockOffset == 0)
            {
                ...
                {
                    var b = readBuffer[i];
                    if (b == '=' || BASE64_DECODE_TABLE[b] != -1) base64Count++;
                }
        ...
        public void Finish()
        {
            ...
            if (m_OffsetInEncode3x8Block == 1)
            {
                ...
                m_pEncodeBuffer[m_EncodeBufferOffset++] =
                    (byte) BASE64_ENCODE_TABLE[(m_pEncode3x8Block[0] & 0x03) << 4];
                m_pEncodeBuffer[m_EncodeBufferOffset++] = (byte) '=';
                m_pEncodeBuffer[m_EncodeBufferOffset++] = (byte) '=';
            }
            else if (m_OffsetInEncode3x8Block == 2)
            {
                m_pEncodeBuffer[m_EncodeBufferOffset++] = (byte) BASE64_ENCODE_TABLE[m_pEncode3x8Block[0] >> 2];
                m_pEncodeBuffer[m_EncodeBufferOffset++] =
                    (byte) BASE64_ENCODE_TABLE[((m_pEncode3x8Block[0] & 0x03) << 4) | (m_pEncode3x8Block[1] >> 4)];
                m_pEncodeBuffer[m_EncodeBufferOffset++] =
                    (byte) BASE64_ENCODE_TABLE[(m_pEncode3x8Block[1] & 0x0F) << 2];
                m_pEncodeBuffer[m_EncodeBufferOffset++] = (byte) '=';
            }

            if (m_EncodeBufferOffset > 0) m_pStream.Write(m_pEncodeBuffer, 0, m_EncodeBufferOffset);
        }
