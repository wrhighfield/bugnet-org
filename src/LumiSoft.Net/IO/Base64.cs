...
namespace LumiSoft.Net.IO
{
    ...
    public class Base64
    {
        ...
        public int Decode(byte[] encBuffer, int encOffset, int encCount, byte[] buffer, int offset,
            bool ignoreNonBase64Chars)
        {
            if (encBuffer == null) throw new ArgumentNullException(nameof(encBuffer));
            if (encOffset < 0)
            ...
            if (encOffset + encCount > encBuffer.Length)
                throw new ArgumentOutOfRangeException(nameof(encCount),
                    "Argument 'count' is bigger than than argument 'encBuffer'.");
            ...
            while (decodeOffset - encOffset < encCount)
            {
                ...
                while (offsetInBlock < 4)
                {
                    ...
                    if (b == '=')
                    {
                        ...
                        // ab==
                        // abc=
                        if (offsetInBlock < 2) throw new FormatException("Invalid base64 padding.");

                        // Skip next padding char.
                        if (offsetInBlock == 2) decodeOffset++;
                    ...
                    {
                        if (!ignoreNonBase64Chars) throw new FormatException("Invalid base64 char '" + b + "'.");
                        // Igonre that char.
                        //else{
                    }
                    // Base64 char.
                ...
                // Decode base64 block.
                if (offsetInBlock > 1) buffer[decodedOffset++] = (byte) ((base64Block[0] << 2) | (base64Block[1] >> 4));
                if (offsetInBlock > 2)
