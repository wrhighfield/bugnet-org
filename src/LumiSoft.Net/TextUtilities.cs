...
namespace LumiSoft.Net
{
    ...
    public class TextUtilities
    {
        ...
        public static string QuoteString(string text)
        {
            ...
            if (!string.IsNullOrWhiteSpace(text) && text.StartsWith("\"") && text.EndsWith("\"")) return text;
        ...
        public static string UnQuoteString(string text)
        {
            ...
            // All text trimmed
            if (endPosInText - startPosInText <= 0) return "";

            // Remove starting and ending quotes.         
            if (text[startPosInText] == '\"') startPosInText++;
            if (text[endPosInText - 1] == '\"') endPosInText--;

            // Just '"'
            if (endPosInText == startPosInText - 1) return "";
        ...
        public static string[] SplitQuotedString(string text, char splitChar, bool unquote, int count)
        {
            ...
            for (var i = 0; i < text.Length; i++)
            {
                ...
                if (splitParts.Count + 1 >= count) break;

                // We have quoted string start/end.
                if (lastChar != '\\' && c == '\"') inQuotedString = !inQuotedString;
        ...
        public static int QuotedIndexOf(string text, char indexChar)
        {
            ...
            for (var i = 0; i < text.Length; i++)
            {
                ...
                if (!inQuotedString && c == indexChar) return i;
            }
        ...
        public static string[] SplitString(string text, char splitChar)
        {
            ...
            // Add last split part to split parts list
            if (lastSplitPoint <= textLength) splitParts.Add(text.Substring(lastSplitPoint));
        ...
        public static bool IsToken(string value)
        {
            ...
            */
            ...
            return (from c in value
                where (c < 0x41 || c > 0x5A) && (c < 0x61 || c > 0x7A) && (c < 0x30 || c > 0x39)
                select tokenChars.Any(tokenChar => c == tokenChar)).All(validTokenChar => validTokenChar);
        }
